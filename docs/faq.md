...
**Compatible relaxation:** Setting `-pc_air_cf_splitting_type cr` switches to a compatible relaxation splitting which measures the quality of $\mathbf{A}_\textrm{ff}$ directly: it starts with all points as F and repeatedly promotes an independent set of the F rows where the relaxed error remains largest, until one application of AIR's own F-point smoothing (an assembled approximate inverse with the settings of `-pc_air_inverse_type`, `-pc_air_poly_order`, `-pc_air_inverse_sparsity_order` and `-pc_air_diag_scale_polys`; always assembled, as that is what the ideal restrictor is built from) contracts a random error on $\mathbf{A}_\textrm{ff}$ by the target rate set by `-pc_air_strong_threshold` (around 0.1 is reasonable — it should reflect the per-application accuracy required of the F-solve; larger values coarsen less and give cheaper but weaker hierarchies). The `-pc_air_max_luby_steps`, `-pc_air_ddc_its` and `-pc_air_ddc_fraction` options are ignored with this CF splitting. Unlike the other splittings it never forms a strength matrix.
...
3) `PCMatApplyTranspose` is not implemented directly for either PC, so a transposed block apply falls back to PETSc applying `PCApplyTranspose` one column at a time. The forward `PCMatApply` does do a real block apply (see above), so a transposed multiple right-hand side solve will be slower than the forward one.
