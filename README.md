...
This library depends on MPI, BLAS, LAPACK and PETSc (>=3.23) configured with a graph partitioner (e.g., ParMETIS).

Please compile PETSc directly from the source code, as PFLARE requires access to some of the PETSc types only available in the source. If you wish to run on GPUs you should use the `release` branch of PETSc and configure PETSc with Kokkos.
