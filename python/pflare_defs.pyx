...
cdef extern:
	...
	void PCAIRGetNumLevels_c(PetscPC *pc, int *input_int)
	...
	void PCAIRGetPolyCoeffs_c(PetscPC *pc, int petsc_level, int which_inverse,
	                           double **coeffs_ptr, int *row_size, int *col_size)

	# PCAIR - set polynomial coefficients (copies from the provided pointer)
	void PCAIRSetPolyCoeffs_c(PetscPC *pc, int petsc_level, int which_inverse,
	                           double *coeffs_ptr, int row_size, int col_size)
	...
	# The Python wrapper copies the data before returning.
	int PCPFLAREINVGetPolyCoeffs(PetscPC pc, double **coeffs, int *rows, int *cols)
...
cpdef cnp.ndarray pcair_get_poly_coeffs(PC pc, int petsc_level, int which_inverse):
	...
	"""
	cdef double *coeffs_ptr = NULL
	cdef int row_size = 0, col_size = 0
	...
	memcpy(<void*>result.data, <void*>coeffs_ptr, row_size * col_size * sizeof(double))
	return result
...
cpdef pcair_set_poly_coeffs(PC pc, int petsc_level, int which_inverse, cnp.ndarray coeffs):
	...
	cdef int row_size = <int>coeffs_f.shape[0]
	cdef int col_size = <int>coeffs_f.shape[1]
	PCAIRSetPolyCoeffs_c(&(pc.pc), petsc_level, which_inverse,
	                      <double*>coeffs_f.data, row_size, col_size)
...
cpdef cnp.ndarray pcpflareinv_get_poly_coeffs(PC pc):
	...
	memcpy(<void*>result.data, <void*>coeffs_ptr, rows * cols * sizeof(double))
	return result
