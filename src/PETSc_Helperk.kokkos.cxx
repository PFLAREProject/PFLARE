...
{
   PflareTraceScope trace_scope("rewrite_j_global_to_local");
   ...
   if (col_ao_output > 0 && j_nonlocal_d.extent(0) > 0)
   {
      PetscInt bad_j = 0;
      Kokkos::parallel_reduce(
         Kokkos::RangePolicy<>(exec, 0, j_nonlocal_d.extent(0)),
         KOKKOS_LAMBDA(const PetscInt i, PetscInt &thread_sum) {
            const PetscInt v = j_nonlocal_d(i);
            if (v < 0 || v >= col_ao_output) thread_sum++;
         },
         bad_j);
      if (bad_j > 0)
      {
         fprintf(stderr,
            "[PFLARE] rewrite_j_global_to_local OOB rewritten indices=%" PetscInt_FMT " (col_ao_output=%" PetscInt_FMT ")\\n",
            bad_j, col_ao_output);
         fflush(stderr);
      }
   }
...
{
   PflareTraceScope trace_scope("remove_small_from_sparse_kokkos");
   ...
   pflare_guard_seq_csr(mat_local, local_cols, MPI_COMM_MATRIX, "remove_small_from_sparse_kokkos", "local");
   if (mpi) pflare_guard_seq_csr(mat_nonlocal, cols_ao, MPI_COMM_MATRIX, "remove_small_from_sparse_kokkos", "nonlocal");
...
{
   PflareTraceScope trace_scope("remove_from_sparse_match_kokkos");
   ...
   pflare_guard_seq_csr(mat_local, local_cols, MPI_COMM_MATRIX, "remove_from_sparse_match_kokkos", "local_in");
   if (mpi) pflare_guard_seq_csr(mat_nonlocal, cols_ao_input, MPI_COMM_MATRIX, "remove_from_sparse_match_kokkos", "nonlocal_in");
   pflare_guard_seq_csr(mat_local_output, local_cols, MPI_COMM_MATRIX, "remove_from_sparse_match_kokkos", "local_out");
   if (mpi) pflare_guard_seq_csr(mat_nonlocal_output, cols_ao_output, MPI_COMM_MATRIX, "remove_from_sparse_match_kokkos", "nonlocal_out");
...
{
   PflareTraceScope trace_scope("MatSetAllValues_kokkos");
   MatType mat_type;
   ...
   pflare_guard_seq_csr(mat_local, local_cols, MPI_COMM_MATRIX, "MatSetAllValues_kokkos", "local");
   if (mpi)
   {
      PetscInt rows_ao = 0, cols_ao = 0;
      PetscCallVoid(MatGetSize(mat_nonlocal, &rows_ao, &cols_ao));
      pflare_guard_seq_csr(mat_nonlocal, cols_ao, MPI_COMM_MATRIX, "MatSetAllValues_kokkos", "nonlocal");
   }
...
{
   PflareTraceScope trace_scope("mat_duplicate_copy_plus_diag_kokkos");
   MPI_Comm MPI_COMM_MATRIX;
   PetscInt global_row_start_temp, global_row_end_plus_one_temp;
   PetscInt global_col_start_temp, global_col_end_plus_one_temp;
   PetscInt rows_ao, cols_ao;
...
{
   PflareTraceScope trace_scope("MatAXPY_kokkos");
   ...
   pflare_guard_seq_csr(mat_local_y, local_cols, MPI_COMM_MATRIX, "MatAXPY_kokkos", "local_y");
   pflare_guard_seq_csr(mat_local_x, local_cols, MPI_COMM_MATRIX, "MatAXPY_kokkos", "local_x");
   pflare_guard_seq_csr(mat_nonlocal_y, cols_ao_y, MPI_COMM_MATRIX, "MatAXPY_kokkos", "nonlocal_y");
   pflare_guard_seq_csr(mat_nonlocal_x, cols_ao_x, MPI_COMM_MATRIX, "MatAXPY_kokkos", "nonlocal_x");
...
PETSC_INTERN void MatCreateSubMatrix_Seq_kokkos(Mat *input_mat, PetscIntKokkosView &is_row_d_d, PetscIntKokkosView &is_col_d_d, const int reuse_int, Mat *output_mat, const char *block_tag = nullptr, const char *part_tag = nullptr)
{
   ...
   Kokkos::parallel_for(
      Kokkos::RangePolicy<>(exec, 0, local_cols_col), KOKKOS_LAMBDA(PetscInt i) {      
         ...
         const PetscInt col_local = is_col_d_d(i);
         if (col_local >= 0 && col_local < local_cols) smap_d(col_local) = i + 1;
   });     
   ...
   if (!reuse_int)
   {
      Kokkos::parallel_reduce(
         ...
         if (i < 0 || i >= local_rows) {
            Kokkos::single(Kokkos::PerTeam(t), [&]() {
               nnz_match_local_row_d(i_idx_is_row) = 0;
            });
            return;
         }
         const PetscInt ncols_local = device_local_i[i + 1] - device_local_i[i];
         ...
         Kokkos::parallel_reduce(
            ...
            [&](const PetscInt j, PetscInt& thread_data) {
               ...
               if (target_col >= 0 && target_col < local_cols && smap_d(target_col))
               {
                  thread_data++;
               }
            }, row_result
         ...
         PetscIntKokkosView &is_col_d_d, PetscInt global_cols_col, const int reuse_int, Mat *output_mat, const char *block_tag)
{
   ...
   if (mpi)
   {
      ...
      if (!reuse_int)
      {
         pflare_diag_stage(MPI_COMM_MATRIX, "MatCreateSubMatrix_kokkos_view", block_tag, "offdiag", "build_noreuse_begin");
         PetscInt isstart = 0;
         ...
         pflare_diag_stage(MPI_COMM_MATRIX, "MatCreateSubMatrix_kokkos_view", block_tag, "offdiag", "after_mpi_scan");
         ...
         if (!x_d_ptr || !cmap_d_ptr || !lvec_d_ptr)
         {
            ...
            MPI_Comm_rank(MPI_COMM_MATRIX, &rank);
            fprintf(stderr,
               "[PFLARE][rank %d] MatCreateSubMatrix_kokkos_view null vec ptr (x=%p,cmap=%p,lvec=%p, block=%s)\n",
         ...
         if (!lcmap_d_ptr)
         {
            ...
            MPI_Comm_rank(MPI_COMM_MATRIX, &rank);
            fprintf(stderr,
               "[PFLARE][rank %d] MatCreateSubMatrix_kokkos_view null lcmap ptr (block=%s)\n",
         ...
         pflare_diag_stage(MPI_COMM_MATRIX, "MatCreateSubMatrix_kokkos_view", block_tag, "offdiag", "after_destroy_x");
         ...
         pflare_diag_stage(MPI_COMM_MATRIX, "MatCreateSubMatrix_kokkos_view", block_tag, "offdiag", "after_find_matches");
         ...
         pflare_diag_stage(MPI_COMM_MATRIX, "MatCreateSubMatrix_kokkos_view", block_tag, "offdiag", "after_match_scan");
         ...
         Kokkos::parallel_for(
            Kokkos::RangePolicy<>(exec, 0, cols_ao), KOKKOS_LAMBDA(PetscInt i) {     
               ...
               exec.fence();
               pflare_diag_stage(MPI_COMM_MATRIX, "MatCreateSubMatrix_kokkos_view", block_tag, "offdiag", "after_pack_iscol_o");
                     ...
                     const int our_level, const int is_row_fine_int, const int is_col_fine_int)
{
   ...
   if (!use_cached_views)
   {
      ...
      // stream-orders all work on the PETSc execution space.
      Kokkos::deep_copy(exec, is_row_d_d, is_row_view_h);     
      Kokkos::deep_copy(exec, is_col_d_d, is_col_view_h);
      // Ensure host pointers from ISGetIndices are no longer needed before ISRestoreIndices.
      exec.fence();
      // Log copy with petsc
